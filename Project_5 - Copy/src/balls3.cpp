...
void bounce_balls(int ball1, int ball2)
{
   ...
   // Move balls so they do not intersect
   float move = (sum_radius - distance) / 2;
   Px[ball1] += dx * move / distance;
   Py[ball1] += dy * move / distance;
   Px[ball2] -= dx * move / distance;
   Py[ball2] -= dy * move / distance;
